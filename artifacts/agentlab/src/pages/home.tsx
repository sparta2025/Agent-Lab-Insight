...
import { getGetTaskQueryKey, getListTasksQueryKey, useCreateTask, useGetTask, useHealthCheck, useListAgents, useListTasks } from '@workspace/api-client-react';
import type { TaskMode, TaskRecord } from '@workspace/api-client-react';
import { ErrorState, EmptyState, ModeBadge, PageHeader, RunDetail, Shell, SkeletonRows, StepPipeline, TaskRow, cn } from '@/components/agentlab-ui';
...
export default function Home() {
  ...
  const selectedTask = useGetTask(selected?.taskId ?? '', {
    query: {
      enabled: Boolean(selected?.taskId),
      queryKey: getGetTaskQueryKey(selected?.taskId ?? ''),
      refetchInterval: (query) => query.state.data?.status === 'running' ? 1000 : false,
    },
  });
  const visibleTask = selectedTask.data ?? selected;
  const isAnalysisRunning = visibleTask?.status === 'running';
    ...
    <PageHeader eyebrow="Analysis workspace" title="Make the unknown inspectable." description="Route a code problem through focused specialists, then review the evidence behind the decision." />
     <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(330px,.92fr)]">
      <section className="rounded-xl border border-border bg-card p-5 card-surface sm:p-6" data-testid="panel-task-composer"><div className="mb-5 flex items-center justify-between"><div><div className="mb-1 flex items-center gap-2 text-sm font-bold"><Plus size={16} className="text-primary" />New analysis</div><p className="text-xs text-muted-foreground">Describe the decision you need help making.</p></div><span className="font-mono-app text-[9px] uppercase tracking-[.15em] text-muted-foreground">Input / 01</span></div>
       ...
       <section className="space-y-5"><div className="rounded-xl border border-border bg-card p-5 card-surface"><div className="mb-5 flex items-center justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.17em] text-primary">Router telemetry</p><h2 className="mt-1 text-base font-bold">Execution pipeline</h2></div><span className={cn('flex items-center gap-1.5 rounded-md border px-2 py-1 font-mono-app text-[9px] font-bold uppercase', health.isError ? 'border-destructive/25 bg-destructive/5 text-destructive' : isAnalysisRunning ? 'border-accent/40 bg-accent/15 text-accent-foreground' : 'border-primary/20 bg-primary/8 text-primary')}><span className={cn('h-1.5 w-1.5 rounded-full bg-current', isAnalysisRunning && 'animate-pulse')} />{health.isLoading ? 'Checking' : health.isError ? 'Offline' : isAnalysisRunning ? 'Running' : 'Ready'}</span></div>{visibleTask ? <StepPipeline steps={visibleTask.steps} /> : <div className="space-y-3">{['Task intake', 'Router selects specialists', 'Parallel analysis', 'Judge synthesis'].map((label, i) => <div className="flex items-center gap-3" key={label}><span className={cn('flex h-7 w-7 items-center justify-center rounded-md font-mono-app text-[10px] font-bold', i === 0 ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground')}>0{i + 1}</span><div className="h-px flex-1 bg-border"><span className={cn('block h-px bg-primary', i === 0 ? 'w-full' : 'w-0')} /></div><span className={cn('text-xs font-semibold', i === 0 ? 'text-foreground' : 'text-muted-foreground')}>{label}</span></div>)}</div>}<p className="mt-5 border-t border-border pt-4 text-[11px] leading-relaxed text-muted-foreground">{isAnalysisRunning ? 'Live status updates every second while specialists and the Judge work.' : 'Each run leaves a traceable record: who analyzed, what they found, and how the Judge arrived at its call.'}</p></div>
     ...
     {visibleTask && <div className="mt-8"><div className="mb-4 flex items-center justify-between"><div><p className="font-mono-app text-[10px] uppercase tracking-[.17em] text-primary">{isAnalysisRunning ? 'Live execution' : 'Latest synthesis'}</p><h2 className="mt-1 text-xl font-bold">{isAnalysisRunning ? 'Analysis in progress' : 'Your analysis, decoded'}</h2></div><button onClick={() => setSelected(null)} className="focus-ring text-xs text-muted-foreground hover:text-foreground" data-testid="button-dismiss-result">Dismiss</button></div><RunDetail task={visibleTask} /></div>}
